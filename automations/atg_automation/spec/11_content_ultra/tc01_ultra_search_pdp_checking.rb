...
=begin
ATG LeapPad Ultra Content: Search and check app information on Catalog + PDP page
=end

describe "LeapPad Ultra - Search SKU and check info on Catalog/PDP page - Env: #{Data::ENV_CONST} - Locale: #{Data::LOCALE_CONST.upcase}" do
  ...
  titles_list.each_hash do |title|
    e_product_search = ultra_atg_app_center_page.get_expected_product_info_search_page title
    e_product_pdp = ultra_atg_app_center_page.get_expected_product_info_pdp_page title

    context "#{count += 1}. SKU = '#{e_product_search[:sku]}' - #{e_product_search[:short_name]}" do
      a_product_search = {}
      status_code = ''
      url = ''
      ...
      context 'Search and check information on Search page' do
        ...
        it "Search by Product Number: #{url}" do
          ...
          product_info = ultra_atg_app_center_page.get_product_info(product_html, e_product_search[:prod_number])
        ...
        it 'Verify Product info displayed successfully' do
          a_product_search = ultra_atg_app_center_page.get_actual_product_info_search_page product_info
        end
      ...
      context 'Product detail page checking' do
        ...
        it 'Go to PDP' do
          ...
          ultra_atg_app_center_page.go_pdp(e_product_pdp[:prod_number])
          ...
          skip_flag = false
          pdp_info = ultra_atg_app_center_page.get_pdp_info
          a_product_pdp = ultra_atg_app_center_page.get_actual_product_info_pdp_page pdp_info
        ...
        if e_product_pdp[:has_credit_link]
          it "Verify Credit link has content: '#{e_product_pdp[:long_name]}'" do
            if Data::ENV_CONST == 'PREVIEW'
              ...
              a_credits_app_title = ultra_atg_app_center_page.get_credits_text
