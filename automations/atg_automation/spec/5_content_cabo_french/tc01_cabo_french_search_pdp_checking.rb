...
=begin
French ATG Cabo Content: Search and check app information on Catalog + PDP page
=end

describe "French/CABO - Search SKU and check PDP info - Env: #{Data::ENV_CONST} - Locale: #{Data::LOCALE_CONST.upcase}" do
  ...
  titles_list.each_hash do |title|
    e_product_search = fr_cabo_atg_app_center_page.get_expected_product_info_search_page title
    e_product_pdp = fr_cabo_atg_app_center_page.get_expected_product_info_pdp_page title
    ...
    context "#{tc_num += 1}. SKU = '#{e_product_search[:sku]}' - #{e_product_search[:short_name]}" do
      ...
      context 'Search and check information on Search page' do
        product_info = {}
        url = CaboAppCenterContent::CONST_CABO_SEARCH_URL % e_product_search[:prod_number]
        ...
        it "Search by Product Number: #{url}" do
          fr_cabo_atg_app_center_page.load(url)
          product_html = fr_cabo_atg_app_center_page.generate_product_html
          product_info = fr_cabo_atg_app_center_page.get_product_info(product_html, e_product_search[:prod_number])
        ...
        it 'Verify Product info displayed successfully' do
          a_product_search = fr_cabo_atg_app_center_page.get_actual_product_info_search_page product_info
        end

        it "Verify Content/Type is '#{e_product_search[:content_type]}'" do
          expect(e_product_search[:content_type]).to include(a_product_search[:content_type])
        end
        ...
        it "Verify Curriculum (Programme) is '#{e_product_search[:curriculum]}'" do
          expect(a_product_search[:curriculum]).to eq(e_product_search[:curriculum])
        end

        it "Verify Age (Âges) is '#{e_product_search[:age]}'" do
      ...
      context 'Product detail page checking' do
        ...
        it 'Go to PDP' do
          ...
          fr_cabo_atg_app_center_page.go_pdp(e_product_pdp[:prod_number])
          ...
          skip_flag = false
          pdp_info = fr_cabo_atg_app_center_page.get_pdp_info
          a_product_pdp = fr_cabo_atg_app_center_page.get_actual_product_info_pdp_page pdp_info
        ...
        it "Verify Curriculum (Programme) is '#{e_product_pdp[:curriculum]}'" do
          expect(a_product_pdp[:curriculum_top]).to eq(e_product_pdp[:curriculum])
        end

        it "Verify Age (Âges) is '#{e_product_pdp[:age]}'" do
        ...
        it "Verify 'Add to Cart' (Ajouter au panier) button displays" do
          expect(a_product_pdp[:add_to_cart_btn]).to eq(e_product_pdp[:add_to_cart_btn])
        end

        it "Verify 'Add to Wishlist' (Ajouter à mes favoris) link displays" do
        ...
        it "Verify Content Type is '#{e_product_pdp[:content_type]}'" do
          expect(e_product_pdp[:content_type]).to include(a_product_pdp[:content_type])
        end
        ...
        it "Verify Curriculum (Programme) bottom is '#{e_product_pdp[:curriculum]}'" do
          expect(a_product_pdp[:curriculum_bottom]).to eq(e_product_pdp[:curriculum])
        end

        it "Verify Publisher (Éditeur) is '#{e_product_pdp[:publisher]}'" do
          expect(a_product_pdp[:publisher]).to eq(e_product_pdp[:publisher])
        end

        it "Verify File Size (Taille) is '#{e_product_pdp[:size]}'" do
        ...
        it "Verify Credit (Crédits) link exists: '#{e_product_pdp[:has_credit_link]}'" do
          expect(a_product_pdp[:has_credit_link]).to eq(e_product_pdp[:has_credit_link])
        end

        if e_product_pdp[:has_credit_link]
          it "Verify Credit (Crédits) text contains long name: '#{e_product_pdp[:credits_text]}'" do
            if Data::ENV_CONST == 'PREVIEW'
              pending "*** Skipped Verify Credit link has content: '#{e_product_pdp[:long_name]}' on PREVIEW env"
            else
              a_credits_app_title = fr_cabo_atg_app_center_page.get_credits_text
              expect(a_credits_app_title.downcase).to include(e_product_pdp[:credits_text].downcase)
            end
          end
        end

        it "Verify Teaches (Apports éducatifs) list #{teaches_and_learning}" do
        ...
        it 'Verify More Like This (Apps similaires) box displays' do
