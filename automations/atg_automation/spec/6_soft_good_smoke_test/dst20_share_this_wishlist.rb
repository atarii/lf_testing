...
feature "DST20 - Wishlist - Share this Wishlist - ENV = '#{Data::ENV_CONST}' - Locale = '#{Data::LOCALE_CONST}'", js: true do
  before :all do
    # Login to LF Account
  ...
  context 'Add 2 items into Wishlist' do
    ...
    scenario '2. Add 1st item into Wishlist' do
      ...
      pending "***2. Add 1st item into Wishlist: PROD_ID = '#{prod_info_1[:id]}'"
    end

    scenario '3. Add 2nd item into Wishlist' do
      ...
      pending "***3. Add the 2nd item into Wishlist: PROD_ID = '#{prod_info_2[:id]}'"
    end
    ...
    scenario '4. Verify the number of item in Wishlist next to \'My Wishlist\' link is 2' do
  ...
  context 'Check Share this Wishlish Email' do
    ...
    scenario '5. Verify 2nd item displays in Email with correct Product ID' do
      expect(wishlist2[:prod_id]).to eq(prod_info_2[:id])
    end

    scenario '6. Verify 2nd item displays in Email with correct Title' do
      if prod_info_2[:title].include? '...'
        prod_info_2[:title] = prod_info_2[:title].delete('...')
        expect(wishlist2[:title]).to include(prod_info_2[:title])
      else
        expect(wishlist2[:title]).to eq(prod_info_2[:title])
      end
    end
  end
