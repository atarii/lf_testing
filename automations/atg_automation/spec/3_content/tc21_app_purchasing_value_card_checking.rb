...
=begin
ATG content: Verify user can search and check out app by using Redeem value code
=end
...
if env != 'PREVIEW' && env != 'PROD'
  ...
  describe "Search SKU and Purchase App using value card pin checking - Env: #{env} - Locale: #{Data::LOCALE_CONST}" do
    ...
    before :all do
      CustomerManagement.clear_account_licenses AppCenterAccount::EMPTY_ACC[0], AppCenterAccount::EMPTY_ACC[1]

      atg_app_center_page.load AppCenterContent::CONST_LOGIN_URL
      atg_login_page.login AppCenterAccount::EMPTY_ACC[0], AppCenterAccount::EMPTY_ACC[1]
    ...
    titles_list.each_hash do |title|
      ...
      context "#{count += 1}. SKU = '#{e_product[:sku]}' - #{e_product[:short_name]}" do
        ...
        context "Purchase using paypal to user '#{AppCenterAccount::EMPTY_ACC[0]}'" do
          it 'Check out process' do
            skip ConstMessage::PRE_CONDITION_FAIL if product_info.empty?
            a_successful_msg = atg_checkout_page.checkout_with_value_card
          end
