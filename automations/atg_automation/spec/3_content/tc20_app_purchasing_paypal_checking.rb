...
=begin
ATG content: Verify user can search and check out app by using PayPal
=end
...
if env != 'PREVIEW' && env != 'PROD'
  ...
  describe "Search SKU and Purchase App using paypal checking - Env: #{env} - Locale: #{Data::LOCALE_CONST}" do
      ...
      context "#{count += 1}. SKU = '#{e_product[:sku]}' - #{e_product[:short_name]}" do
        ...
        context "Purchase using paypal to user '#{AppCenterAccount::EMPTY_ACC[0]}'" do
          it 'Check out process' do
            ...
            a_successful_msg = atg_checkout_page.checkout_with_paypal
          end
