...
=begin
ATG Content: Open Quick View overlay and check app information
=end

describe "Check information on Quick View page - Env: #{Data::ENV_CONST} - Locale: #{Data::LOCALE_CONST.upcase}" do
  ...
  titles_list.each_hash do |title|
    ...
    context "#{count += 1}. SKU = '#{e_product[:sku]}' - #{e_product[:short_name]}" do
      skip_flag = false
      url = AppCenterContent::CONST_QUICK_VIEW_SEARCH_URL % e_product[:prod_number]
